...
2026-09-05 · **Salto por FK no RESULTADO de query arbitrária ficou de fora — precisa da origem da coluna.** A fatia de navegação por FK pediu o salto "na aba Dados E no resultado de query". Fiz: a aba Dados (tabela real, mapeamento coluna→FK inambíguo) e o painel "Tabela" da aba de consulta (também a tabela de origem). **Não fiz** o grid do resultado de um SQL arbitrário: um `SELECT` com join/alias/coluna computada não deixa saber, com segurança, que uma coluna de resultado é a FK de alguma tabela. Casar por nome de coluna erraria no join (abriria a referenciada errada, filtrada por um valor de outra coluna) — "esperto e frágil". O caminho correto exige capturar a **origem** de cada coluna do resultado: o `pg` expõe `tableID` e `columnID` na RowDescription (0 quando a coluna não vem direto de uma tabela). Fatia própria: o executor passa a devolver `tableID`/`columnID` por coluna (shared `ResultColumn`), o front resolve `tableID`→schema.tabela e mapeia para as FKs daquela tabela — aí o salto no resultado fica correto mesmo com join. Até lá, o salto vale para tabela e para o painel de origem, que é o caso comum.
